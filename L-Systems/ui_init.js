...
function init_ui(){
  ...
  let sldr_initLen = createSlider(10, 400, 0, 5).style('width', "120px").parent("init_len").input(()=>lsys.setInitLen(sldr_initLen.value()));
  ...
  let sldr_initLenFac = createSlider(0.05, 2.0, 0, 0.05).style('width', "120px").parent("init_lenFac").input(()=>lsys.setLenFac(sldr_initLenFac.value()));


  let p_initRot = createElement("p", "Init Rotation: ").id("init_rot").class("ui_subl").parent("ui");
  let sldr_initRot = createSlider(0, TWO_PI, 0, PI/60).style('width', "120px").parent("init_rot").input(()=>lsys.setInitRot(sldr_initRot.value()));
