...
function setup(){
  ...
  agent     = new PathGridWalker(11,11,cellSize);
  initUI(); 
} // Ends Function setup
...
|              'Agent'/'Gridwalker' classes of TD-P5JS/ZAC-MVP families.
+-----------------------------------------------------------------------
|> Special [Pre-]Archival Note: If/when merging this project's code into
|  either a [larger] side project else the 'TRUE universal' definition 
|  of a [Gridwalker] Agent; do refer to and provide PRIORITY towards the
|  following methods per their features/qualities; as to integrate them
|  within the larger domain (unless they're already added, of course)...
|  (1) <render> : As it's the FIRST use of my improved render method for
|                 triangle/chevron agents sans where it was created.
|  (2) <update> : As a 'backup' in case there are [unknown] bugs in the
|                 Gridwalker code of 'P5JS Gridwalker' and/or otherwise.
|  (3) [VERY] new method for realizing 'turn to face waypoint zero if it
|      is immediately behind the agent, <vs> 'insta-spin. Further Notes:
|  > numRots: Number of steps to turn 180 degrees s.t. [numRots=30] => 
|             [30] frames with [180/30]=[6] degree rotations per frame.
|             ALSO: I've related it to maxForce with a heuristic which
|             seems to work; but manual assignment is always an option.
+=====================================================================*/
class PathGridWalker{
  constructor(r,c,l){
    //> Transform
    this.pos = createVector(c*l+(l/2),r*l+(l/2));
    ...
    this.rotDone = false;
    this.numRots = round(PI/this.maxForce)*2;
    this.curRots =  0;
    //> GFX/VFX Settings
    this.bLen = l; this.bHalf = this.bLen/2; this.bSxth = this.bLen/6; this.bThrd = this.bLen/3;
    this.fill = color(60); this.strk = color(0,255,0,255); this.sWgt = 1.5;
  }
  ...
  update(){
    if(this.curWaypt>=this.curPath.length){return;}
    if(this.curWaypt==0 && !this.rotDone){this.rotateToFirstWayPt();}
    else {this.gridWalkAlongPath();}
  } // Ends Function gotoPath

  rotateToFirstWayPt(){
    if(this.curRots==0 && abs(p5.Vector.dot(this.ori.copy().normalize(),p5.Vector.sub(this.pos,this.curPath[this.curWaypt]).normalize())-1)>=0.98){this.rotDone=true; return;}
    this.ori.setHeading(this.ori.heading()+(PI/this.numRots)); this.curRots++;
    if(this.curRots==this.numRots){this.rotDone=true;}
  } // Ends Function rotateToFirstWayPt
  ...
  render(){fill(this.fill); stroke(this.strk); strokeWeight(this.sWgt); push(); translate(this.pos.x,this.pos.y); rotate(this.ori.heading()); translate(this.bSxth,0); beginShape(); vertex(this.bHalf,0); vertex(-this.bHalf,-this.bHalf); vertex(-this.bThrd,0); vertex(-this.bHalf,this.bHalf); endShape(CLOSE); pop();}  
} // Ends Class PathGridWalker
