class QTNodeDest{
  ...
  collide(diskPos, diskRad){  
    ...
    if(this.qDiskSurrounds(diskPos,diskRad)){this.setToDestroyed();return;}
    ...
    if(cand_NW.qDiskIntersects(diskPos,diskRad)){if(this.NW==null){this.NW = cand_NW;} this.NW.collide(diskPos, diskRad);}
    if(cand_NE.qDiskIntersects(diskPos,diskRad)){if(this.NE==null){this.NE = cand_NE;} this.NE.collide(diskPos, diskRad);}
    if(cand_SW.qDiskIntersects(diskPos,diskRad)){if(this.SW==null){this.SW = cand_SW;} this.SW.collide(diskPos, diskRad);}    
    if(cand_SE.qDiskIntersects(diskPos,diskRad)){if(this.SE==null){this.SE = cand_SE;} this.SE.collide(diskPos, diskRad);}
    
    // Quadrant Traversals yield all quadrants destroyed -> Nullify quadrants and assign state to destroyed
    if(this.NW&&this.NW.destroyed&&this.NE&&this.NE.destroyed&&this.SW&&this.SW.destroyed&&this.SE&&this.SE.destroyed){this.setToDestroyed();}
  } // Ends Function Collide


  // Sets this node to destroyed (i.e. all children = null, destroyed = false)
  setToDestroyed(){
  ...
  render(){
    ...
    if(this.destroyed){fill(this.fill_isDest);}else{fill(this.fill_noDest);}
    if(!this.dispEdges.get()){noStroke();}else{stroke(60);strokeWeight(1);}
